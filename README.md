...
겹친 Gateway resume와 guild unavailable 이벤트는 가장 최근에 관찰한 guild outage를 기준으로 처리합니다. 서버가 아직 unavailable이면 해당 outage 시각까지 음성 기록을 닫고 수집을 중단한 채 기다리며, 실제 available 상태가 확인된 뒤에만 현재 음성 상태와 SoD/EoD delta를 복구합니다. 관리자가 `/활동설정 과거동기화`를 성공시키면 대기 중이던 자동 재시도와 부분 데이터 임시 표시는 함께 정리됩니다.
